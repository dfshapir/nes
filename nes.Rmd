...
# How similar/different are these predictions? pred_gender on partyid7
...
```{r}
plot <- ggplot(y, aes(x = pred_gender, y = partyid7)) +
...
fit_4 <- stan_glm(data = x, partyid7 ~ gender + race, refresh = 0)

new <- x %>%
  mutate(new_idea = real_ideo - 1)

fit_5 <- stan_glm(data = new, partyid7 ~ gender + race + new_idea, refresh = 0)
